...
class BetterGravityInstallerApp(Gtk.Application):
    ...
    def do_activate(self):
        ...
        self.ver_lbl = Gtk.Label(label="v3.0.4")
    ...
    def sync_bootstrapper(self):
        ...
        try:
            req = urllib.request.Request(manifest_url, headers={"User-Agent": "BetterGravity-Bootstrapper-Linux/3.0.4"})
            with urllib.request.urlopen(req, timeout=5) as response:
                if response.status == 200:
                    data = json.loads(response.read().decode("utf-8"))
                    version = data.get("version", "3.0.4")
                    os.makedirs(runtime_dir, exist_ok=True)
                    GLib.idle_add(self.update_bootstrapper_ui, version, "LATEST")
                    return
        except Exception:
            pass
        GLib.idle_add(self.update_bootstrapper_ui, "3.0.4", "OFFLINE")
